...
describe("Detail E2E (Detalle de Noticia)", () => {
  beforeAll(async () => {
    ...
    server = app.listen(0)
    // Lanza Puppeteer en modo headless (sin interfaz gráfica)
  ...
  test(
    "Renderiza correctamente el detalle de la noticia con ID 1",
    async () => {
      ...
      // Validar título
      const title = await page.$eval(".detalle-title", (el) => el.textContent?.trim())
      expect(title).toBe("THE NEXUS BATTLE - Viszla")

      // Validar descripción
      const description = await page.$eval(".detalle-desc", (el) => el.textContent?.trim())
      expect(description).toContain("Grupo de proyecto integrador 2")

      // Validar contenido principal
      const content = await page.$eval(".detalle-content p", (el) => el.textContent?.trim())
      expect(content).toContain(
        "El grupo Vizsla se encargó del módulo de usuarios para el juego NEXUS BATTLE IV"
      )

      // Validar imagen
      const imageSrc = await page.$eval(".detalle-img", (el) => el.getAttribute("src"))
